...
export function ensureAdminUser() {
    ...
    const id = nanoid(12);
    const ts = nowIso();
    db.prepare(`INSERT INTO users (id, username, display_name, email, role, password_hash, active, created_at, updated_at)
     VALUES (?, ?, ?, NULL, 'admin', ?, 1, ?, ?)`).run(id, config.adminUser, "Administrator", hashPassword(config.adminPassword), ts, ts);
}
...
export function createUser(input, actor) {
    ...
    const ts = nowIso();
    db.prepare(`INSERT INTO users (id, username, display_name, email, role, password_hash, active, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, input.username, input.displayName, input.email ?? null, input.role, hashPassword(input.password), input.active ? 1 : 0, ts, ts);
    audit(actor, "user.create", { id, username: input.username, role: input.role });
    return getUserById(id);
}
export function updateUser(id, input, actor) {
    ...
    db.prepare(`UPDATE users SET display_name=?, email=?, role=?, password_hash=?, active=?, updated_at=? WHERE id=?`).run(displayName, email, role, passwordHash, active, nowIso(), id);
...
export function updateProfile(userId, input) {
    ...
    db.prepare(`UPDATE users SET display_name=?, email=?, password_hash=?, updated_at=? WHERE id=?`).run(input.displayName ?? existing.display_name, input.email !== undefined ? input.email : existing.email, passwordHash, nowIso(), userId);
