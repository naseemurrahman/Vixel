...
function bytesOutSum() {
    const row = db
        .prepare(`SELECT COALESCE(SUM(bytes_out), 0) AS s FROM recordings WHERE bytes_out IS NOT NULL`)
        .get();
    return Number(row.s) || 0;
}
function avgCompressionPct() {
    const row = db
        .prepare(`SELECT AVG(compression_ratio) AS a FROM recordings
...
export function collectSample() {
    ...
    try {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(sample.ts, sample.cpuPct, sample.memUsedMb, sample.memRssMb, sample.activeJobs, sample.camerasEnabled, sample.bytesOutRate, sample.compressionPct, sample.diskRecordingsMb);
        db.prepare(`DELETE FROM metric_samples WHERE id NOT IN (SELECT id FROM metric_samples ORDER BY id DESC LIMIT ?)`).run(MAX_SAMPLES);
    }
    catch {
...
export function getRecentSamples(limit = 90) {
    if (samples.length)
        return samples.slice(-limit);
    const rows = db
        .prepare(`SELECT ts, cpu_pct AS cpuPct, mem_used_mb AS memUsedMb, mem_rss_mb AS memRssMb,
...
export function getUsageSummary() {
    ...
    const byDay = db
       ...
       ORDER BY day`)
        .all();
    const byCamera = db
        .prepare(`SELECT c.name AS camera, COUNT(r.id) AS recordings,
       ...
       ORDER BY bytesOut DESC`)
        .all();
    const uploads = db
        .prepare(`SELECT status, COUNT(*) AS c FROM uploads GROUP BY status`)
        .all();
    ...
    return {
        ...
        recordings: {
            ...
            avgCompressionPct: recStats.avgRatio != null ? Math.round(recStats.avgRatio * 1000) / 10 : 0,
        },
        disk: {
            recordingsMb: disk,
            hostFreeMb: freeMb,
