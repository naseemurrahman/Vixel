...
import { z } from "zod";
import { COMPRESSION_PROFILES, profileDefaults, } from "./compression-strategy.js";
...
export function ensureCameraColumns() {
...
function resolveEncodeSettings(input) {
    const profile = input.compressionProfile;
...
export function listCameras() {
    return db.prepare(`SELECT * FROM cameras ORDER BY name`).all();
}
export function getCamera(id) {
    return db.prepare(`SELECT * FROM cameras WHERE id = ?`).get(id);
}
export function countCameras() {
    const row = db.prepare(`SELECT COUNT(*) AS c FROM cameras`).get();
    return row.c;
}
export function createCamera(input, actor) {
    const entitlements = effectiveEntitlements();
    if (countCameras() >= entitlements.maxCameras) {
        throw new Error(`License limit reached: max ${entitlements.maxCameras} camera(s). Upgrade entitlements to add more.`);
    }
    const enc = resolveEncodeSettings(input);
    const id = nanoid(12);
    const ts = nowIso();
    db.prepare(`INSERT INTO cameras (
     ...
     ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, input.name, input.rtspUrl, input.enabled ? 1 : 0, input.segmentSeconds, enc.crf, input.codec, input.preset, input.audio ? 1 : 0, enc.profile, enc.gopSize, enc.bframes, enc.mpdecimate ? 1 : 0, ts, ts);
    audit(actor, "camera.create", { id, name: input.name, profile: enc.profile });
    return getCamera(id);
}
export function updateCamera(id, input, actor) {
    ...
    db.prepare(`UPDATE cameras SET
     ...
     WHERE id=?`).run(merged.name, merged.rtspUrl, merged.enabled ? 1 : 0, merged.segmentSeconds, enc.crf, merged.codec, merged.preset, merged.audio ? 1 : 0, enc.profile, enc.gopSize, enc.bframes, enc.mpdecimate ? 1 : 0, nowIso(), id);
    audit(actor, "camera.update", { id, profile: enc.profile });
    return getCamera(id);
}
export function deleteCamera(id, actor) {
    db.prepare(`DELETE FROM cameras WHERE id = ?`).run(id);
    audit(actor, "camera.delete", { id });
}
export function toPublicCamera(c) {
    const profile = (c.compression_profile || "zipstream");
    return {
        id: c.id,
        name: c.name,
        rtspUrl: c.rtsp_url,
