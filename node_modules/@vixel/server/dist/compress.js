...
import { buildEncodeArgs, buildStrategy, compressionRatio, } from "./compression-strategy.js";
...
function runFfmpeg(args, onProc) {
    return new Promise((resolve, reject) => {
        ...
        proc.on("close", (code) => {
            if (code === 0)
                ...
                reject(new Error(`ffmpeg exited ${code}: ${stderr || "no output"}`));
        });
    });
}
function strategyFromCamera(camera) {
    const profile = (camera.compression_profile || "zipstream");
    return {
        codec: camera.codec,
 ...
 */
export async function compressSegment(camera) {
    ...
    db.prepare(`INSERT INTO recordings (id, camera_id, status, source_path, output_path, started_at)
     VALUES (?, ?, 'capturing', ?, ?, ?)`).run(recordingId, camera.id, rawPath, outPath, nowIso());
    const track = (proc) => {
...
async function uploadToAllTargets(recordingId, cameraId, localFile) {
...
export function stopCameraJob(cameraId) {
...
export function syncCameraLoops() {
...
export function listRecordings(limit = 50) {
    return db
        .prepare(`SELECT r.*, c.name AS camera_name FROM recordings r
